...
class MathQuiz {
    ...
    randomize() {
        const answer = this.answers[this.correctIndex];
        const answers = this.answers.slice();
        this.answers.splice(0, this.answers.length);

        while (answers.length) {
            const index = rand(0, answers.length);
            this.answers.push(answers[index]);
            answers.splice(index, 1);
        }
        this.correctIndex = this.answers.findIndex(a => a === answer);
