...
class JobWorker {
  ...
  _FindAvailableJob() {
    for (let e = 0, s = self.jobQueue.length; e < s; ++e) {
      ...
      if (
        !(
          "number" == typeof t["maxWorkerNum"] &&
          this._number >= t["maxWorkerNum"]
        ) &&
        (!t["isBroadcast"] ||
          (this._number < t["doneFlags"].length &&
            !t["doneFlags"][this._number]))
      )
        return e;
    }
    return -1;
  ...
  if ("_cancel" === t) CancelJob(s.jobId);
    ...
    if ((self.jobQueue.push(s), s["isBroadcast"])) {
      const b = s["maxWorkerNum"],
        ...
        c =
          ((s["doneFlags"] = new Array(p).fill(!1)),
          (s["transferables"] = []),
