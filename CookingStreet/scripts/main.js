...
self.AudioDOMHandler = class extends self.DOMHandler {
    ...
    Tick() {
        ...
        b = this._audioInstances.filter(c => c.IsActive()).map(c =>
            c.GetState());
    ...
    async _Preload(a) {
        ...
        try {
            ...
            console.error("[Construct] Audio: error preloading: ",
                e)
        }
    }
    async _Unload(a) {
    ...
    async _AddEffect(a) {
        ...
        else if ("tremolo" === b) d = new self.C3AudioTremoloFX(this,
            ...d);
    ...
    _DoPostUpdatedFxState() {
        const a = {};
        for (const [b, c] of this._effects) a[b] =
            c.map(d => d.GetState());
