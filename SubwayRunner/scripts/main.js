...
self.RuntimeInterface.AddDOMHandlerClass(class extends self.DOMHandler {
    constructor(a) {
        super(a, "instant-games");
        this._isAvailable = this._hasInitialized = !1;
        this._loadedRewardedVideoAd = this._loadedInterstitialAd = this._FBInstant = null;
        this.AddRuntimeMessageHandlers([
            ["load", b => this._OnLoad(b)],
            ["loading-progress", b => this._OnLoadingProgress(b)],
            ["start-game", () => this._OnStartGame()],
            ["quit", () => this._OnQuit()],
            ["set-player-data", b => this._OnSetPlayerData(b)],
            ["load-player-data", () => this._OnLoadPlayerData()],
            ["share", b => this._OnShare(b)],
            ["custom-update", b => this._OnCustomUpdate(b)],
            ["switch-game", b => this._OnSwitchGame(b)],
            ["change-context", b => this._OnChangeContext(b)],
            ["log-event", b => this._OnLogEvent(b)],
            ["load-connected-players", () => this._OnLoadConnectedPlayers()],
            ["set-score", b => this._OnSetScore(b)],
            ["load-player-score", b => this._OnLoadPlayerScore(b)],
            ["share-leaderboard-update", b => this._OnShareLeaderboardUpdate(b)],
            ["load-leaderboard", b => this._LoadLeaderboard(b)],
            ["create-shortcut", () => this._OnCreateShortcut()],
            ["subscribe-bot", () => this._OnSubscribeBot()],
            ["load-ad", b => this._OnLoadAd(b)],
            ["show-ad", b => this._OnShowAd(b)]
        ])
    }
    async _OnLoad(a) {
        if ("instant-games" !== a.exportType) return console.warn("[Instant Games] Project has not been exported with the 'Instant Games' export option, so Instant Games features will be unavailable"), !1;
        this._FBInstant = window.FBInstant;
        if (!this._FBInstant) return !1;
        await Promise.race([this._InitializeFBInstant(), Wait(4E3)]);
        (this._isAvailable = this._hasInitialized) ? console.info("[Instant Games] Initialized OK"):
    ...
    async _InitializeFBInstant() {
        try {
            await this._FBInstant.initializeAsync(), this._hasInitialized = !0
        } catch (a) {
            console.error("[Instant Games] Failed to initialize: ", a)
        }
    }
    _OnLoadingProgress(a) {
        this._isAvailable && this._FBInstant.setLoadingProgress(a.progress)
    }
    async _OnStartGame() {
        if (!this._isAvailable) return null;
        const a = this._FBInstant;
        await a.startGameAsync();
        a.onPause(() =>
            this.PostToRuntime("pause"));
        const b = a.context,
            c = a.player,
            d = a.getEntryPointData();
        let e = "";
        d && d.hasOwnProperty("data") && (e = d.data);
        return {
            locale: a.getLocale(),
            platform: a.getPlatform(),
            sdkVersion: a.getSDKVersion(),
            contextId: b.getID(),
            contextType: b.getType(),
            playerName: c.getName(),
            playerPhoto: c.getPhoto(),
            playerId: c.getID(),
            entryPointData: e,
            supportedApis: a.getSupportedAPIs()
        }
    }
    PostError() {
    ...
    _OnQuit() {
        this._isAvailable && this._FBInstant.quit()
    }
    async _OnSetPlayerData(a) {
        if (this._isAvailable) try {
            await this._FBInstant.player.setDataAsync({
                data: a.data
            })
        } catch (b) {
            console.error("[Instant Games] Failed to set player data: ",
                b), this.PostError()
        }
    }
    async _OnLoadPlayerData() {
        ...
        try {
            const a = await this._FBInstant.player.getDataAsync(["data"]);
            if (a && a.hasOwnProperty("data")) return a.data
        } catch (a) {
            console.error("[Instant Games] Failed to load player data: ", a), this.PostError()
        }
        return ""
    }
    async _OnShare(a) {
        if (this._isAvailable) try {
            ...
            console.error("[Instant Games] Failed to share: ", b), this.PostError()
        }
    }
    async _OnCustomUpdate(a) {
        if (this._isAvailable) try {
            const b = {
                    action: "CUSTOM",
                ...
                c = a.cta;
            c && (b.cta = c);
            await this._FBInstant.updateAsync(b)
        } catch (b) {
            console.error("[Instant Games] Error posting custom update: ", b)
        }
    }
    async _OnSwitchGame(a) {
        if (this._isAvailable) try {
            await this._FBInstant.switchGameAsync(a.appId, {
                data: a.data
            })
        } catch (b) {
            console.error("[Instant Games] Failed to switch game: ", b), this.PostError()
        }
    }
    async _OnChangeContext(a) {
        if (!this._isAvailable) return {
            isOk: !1
        };
        try {
            const b = {},
                c = a.filter;
            1 === c ? b.filters = ["NEW_CONTEXT_ONLY"] : 2 === c ? b.filters = ["INCLUDE_EXISTING_CHALLENGES"] : 3 === c && (b.filters = ["NEW_PLAYERS_ONLY"]);
            const d = a.minSize,
                e = a.maxSize;
            0 <= d && (b.minSize = d);
            0 <= e && (b.maxSize = e);
            const f = this._FBInstant.context;
            await f.chooseAsync(b);
            return {
                isOk: !0,
                newContextId: f.getID(),
                newContextType: f.getType()
            }
        } catch (b) {
            if ("USER_INPUT" === b.code) return {
                isOk: !1,
                cancelled: !0
            };
    ...
    _OnLogEvent(a) {
        this._isAvailable &&
            this._FBInstant.logEvent(a.name, a.valueToSum)
    }
    async _OnLoadConnectedPlayers() {
        ...
        try {
            return {
                isOk: !0,
                connectedPlayers: (await this._FBInstant.player.getConnectedPlayersAsync()).map(a => ({
                    id: a.getID(),
                    name: a.getName(),
                    photo: a.getPhoto()
                }))
            ...
            return console.error("[Instant Games] Failed to share: ", a), this.PostError(), {
                isOk: !1
            }
        }
    }
    async _OnSetScore(a) {
        ...
        try {
            return await (await this._FBInstant.getLeaderboardAsync(a.leaderboardId)).setScoreAsync(a.score), !0
        } catch (b) {
            return console.error("[Instant Games] Failed to set score: ", b), this.PostError(), !1
        }
    }
    async _OnLoadPlayerScore(a) {
        if (!this._isAvailable) return {
            isOk: !1
        };
        try {
            const b = await (await this._FBInstant.getLeaderboardAsync(a.leaderboardId)).getPlayerEntryAsync();
            if (!b) throw Error("player has no score");
            return {
                isOk: !0,
                score: b.getScore(),
                rank: b.getRank()
            ...
            return console.error("[Instant Games] Failed to load player score: ", b), this.PostError(), {
                isOk: !1
            }
        }
    }
    async _OnShareLeaderboardUpdate(a) {
        if (this._isAvailable) try {
            await this._FBInstant.updateAsync({
                action: "LEADERBOARD",
                name: a.leaderboardId
            })
        } catch (b) {
            console.error("[Instant Games] Failed to share leaderboard update: ", b)
        }
    }
    async _LoadLeaderboard(a) {
        if (!this._isAvailable) return {
            isOk: !1
        };
        const b = a.count,
            c = a.offset;
        try {
            const d = await this._FBInstant.getLeaderboardAsync(a.leaderboardId);
            return {
                isOk: !0,
                leaderboardEntries: (0 === a.results ? await d.getEntriesAsync(b, c) : await d.getConnectedPlayerEntriesAsync(b, c)).map(e => ({
                    score: e.getScore(),
                    rank: e.getRank(),
            ...
            return console.error("[Instant Games] Failed to load leaderboard: ", d), this.PostError(), {
                isOk: !1
            }
        }
    }
    async _OnCreateShortcut() {
        if (!this._isAvailable) return !1;
        try {
            if (!await this._FBInstant.canCreateShortcutAsync()) return !1;
            await this._FBInstant.createShortcutAsync();
            return !0
        } catch (a) {
            return console.error("[Instant Games] Failed to create shortcut: ", a), !1
        }
    }
    async _OnSubscribeBot() {
        if (this._isAvailable) {
            var a = this._FBInstant.player;
            try {
                if (!await a.canSubscribeBotAsync()) throw Error("not allowed by canSubscribeBotAsync()");
                await a.subscribeBotAsync();
    ...
    async _OnLoadAd(a) {
        if (!this._isAvailable) return !1;
        const b = a.placementId;
        try {
            const c = 0 === a.type;
            let d;
            d = c ? await this._FBInstant.getInterstitialAdAsync(b) : await this._FBInstant.getRewardedVideoAsync(b);
            await d.loadAsync();
            c ? this._loadedInterstitialAd = d : this._loadedRewardedVideoAd = d;
            return !0
        } catch (c) {
            return console.error("[Instant Games] Failed to load ad: ",
                c), !1
        }
    }
    async _OnShowAd(a) {
        if (!this._isAvailable) return !1;
        a = 0 === a.type ? this._loadedInterstitialAd : this._loadedRewardedVideoAd;
        if (!a) return !1;
        try {
...
self.RuntimeInterface.AddDOMHandlerClass(class extends self.DOMHandler {
    ...
    _GetActiveGamepads() {
        try {
            ...
            return didLogGetGamepadsError || (console.warn("[Construct] Failed to access gamepads: ", a), didLogGetGamepadsError = !0), []
        }
    }
