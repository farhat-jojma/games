  ...
  const b = class extends self.DOMHandler {
    ...
    async _GetSafeAreaInset() {
      ...
      if (!e) {
        const t = document.body,
          s = t.style,
          n =
            (s.setProperty("--temp-sai-top", "env(safe-area-inset-top)"),
            s.setProperty("--temp-sai-right", "env(safe-area-inset-right)"),
            s.setProperty("--temp-sai-bottom", "env(safe-area-inset-bottom)"),
            s.setProperty("--temp-sai-left", "env(safe-area-inset-left)"),
            getComputedStyle(t)),
          o = [
            n.getPropertyValue("--temp-sai-top"),
            n.getPropertyValue("--temp-sai-right"),
            n.getPropertyValue("--temp-sai-bottom"),
            n.getPropertyValue("--temp-sai-left"),
          ].map((e) => {
            const t = parseInt(e, 10);
            return isFinite(t) ? t : 0;
          });
        return (
          s.removeProperty("--temp-sai-top"),
          s.removeProperty("--temp-sai-right"),
          s.removeProperty("--temp-sai-bottom"),
          s.removeProperty("--temp-sai-left"),
          o
        );
      }
      try {
        return await Promise.all([
          GetAndroidInsetTop(e),
          GetAndroidInsetRight(e),
          GetAndroidInsetBottom(e),
          GetAndroidInsetLeft(e),
        ]);
      } catch (e) {
        return (
          console.error(
    ...
    async _OnRequestWakeLock() {
      if (!this._screenWakeLock)
        try {
          (this._screenWakeLock = await navigator["wakeLock"]["request"](
            "screen"
          )),
