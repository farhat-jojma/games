...
async function LoadScripts(e) {
  if (1 === e.length) {
    const t = e[0];
    await import((IsRelativeURL(t) ? "./" : "") + t);
  } else {
    const i = e
        .map((t) => `import "${IsRelativeURL(t) ? "./" : ""}${t}";`)
        .join("\n"),
    ...
    try {
      ...
      for (const n of e) await import((IsRelativeURL(n) ? "./" : "") + n);
    }
  }
}
async function InitRuntime(t) {
  ...
  if (a)
    try {
      if (
        (await LoadScripts([s]),
        "preview" === i && !globalThis.C3_ProjectMainScriptOK)
      )
      ...
      e.postMessage({ type: "alert-error", message: l });
    }
  if (o)
    try {
      if (
        (await LoadScripts([c]),
        "preview" === i && !globalThis.C3.ScriptsInEvents)
      )
  ...
  (t["runOnStartupFunctions"] = r),
    e.postMessage({ type: "creating-runtime" }),
    (runtime = self["C3_CreateRuntime"](t)),
    await self["C3_InitRuntime"](runtime, t);
}
