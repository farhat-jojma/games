...
async function UpdateCheck(e) {
  try {
    ...
    if (!t.ok)
      throw new Error(
        OFFLINE_DATA_FILE + " responded with " + t.status + " " + t.statusText
      );
    ...
    if (i) {
      ...
      const l = await GetMainPageUrl(),
        d =
          (s.unshift("./"),
          l && -1 === s.indexOf(l) && s.unshift(l),
          console.log(
            CONSOLE_PREFIX + "Caching " + s.length + " files for offline use"
          ),
          e ? Broadcast("downloading") : BroadcastDownloadingUpdate(o),
          n && (await WriteLazyLoadListToStorage(n)),
          await CreateCacheFromFileList(r, s, !e),
          await IsUpdatePending());
      d
        ? (console.log(CONSOLE_PREFIX + "All resources saved, update ready"),
          BroadcastUpdateReady(o))
        : (console.log(
...
async function HandleFetch(t, e) {
  ...
  if (IsUrlInLazyLoadList(t.request.url, c))
    try {
      ...
      console.warn(
        CONSOLE_PREFIX + "Error caching '" + t.request.url + "': ",
        e
      );
    }
  return i;
