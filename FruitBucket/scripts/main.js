...
window.RuntimeInterface = class a {
  ...
  async _InitWorker(b, c) {
    ...
    this._canvasLayers.push({ canvas: e, htmlWrap: g });
    window.c3canvas = e;
    ...
    self.c3_callFunction = (l, m) =>
      this._runtimeDomHandler._InvokeFunctionFromJS(l, m);
    "preview" === this._exportType &&
  ...
  async _InitDOM(b, c) {
    ...
    e = b.engineScripts.map((h) =>
      "string" === typeof h ? new URL(h, this._runtimeBaseUrl).toString() : h
    );
    if (Array.isArray(b.workerDependencyScripts)) {
      var f = [...b.workerDependencyScripts].map((h) =>
        h instanceof Blob ? URL.createObjectURL(h) : h
      );
      e.unshift(...f);
    }
    ...
    for (let [h, k] of g)
      if ((k || (k = h), h === f))
        ...
        (k = await this._MaybeGetCordovaScriptURL(k)), await AddScript(k);
    "preview" === this._exportType &&
  ...
  _OnSetHTMLLayerCount(b) {
    ...
    if (c < this._canvasLayers.length)
      ...
      for (let l = 0, m = c - this._canvasLayers.length; l < m; ++l)
        (c = document.createElement("canvas")),
          ...
          this._useWorker
            ? ((d = c.transferControlToOffscreen()), h.push(d), k.push(d))
            : h.push(c),
    ...
    return { addedCanvases: h, transferables: k };
  }
  ...
  _DoUpdateHTMLElementsZOrderOnHTMLLayer(b, c) {
    if (!(1 >= c.length || b >= this._canvasLayers.length)) {
      ...
      for (
        var e = Array.from(b.children).filter((k) => d.has(k)),
          f = 0,
          g = Math.min(c.length, e.length);
        f < g && c[f] === e[f];
        ++f
      );
      for (var h = f; h < g; ++h) e[h].remove();
      for (h = f; h < g; ++h) b.appendChild(c[h]);
    }
  }
  _GetLocalRuntime() {
...
class C3BannerAd {
  constructor(a, b, c, d, e) {
    if (!a) return Failure$jscomp$1(e, "Unit ID not specified");
    if (!b) return Failure$jscomp$1(e, "Ad size not specified");
    if (!c) return Failure$jscomp$1(e, "Ad position not specified");
    this._adUnitId = a;
    this._position = c;
    this._size = "";
    this._offset = d;
...
addHandler("CreateBannerAdvert", async (a, b, c, d, e, f, g) => {
  (await C3AdUtils$jscomp$5.StartAdmobPlus(g, c)) &&
    (e &&
      bannerAd &&
...
self.RuntimeInterface.AddDOMHandlerClass(
  class extends self.DOMElementHandler {
    constructor(a) {
      super(a, "video");
      this.SetAutoAttach(!1);
      this._postImageBitmaps =
        this._supportsRvfc =
        this._isRuntimeInWorker =
          !1;
      this._numVideos = this._sequenceNumber = 0;
      this._destroyedVideos = new WeakSet();
      this.AddRuntimeMessageHandler("init", (b) => this._OnInit(b));
      this.AddDOMElementMessageHandler("play", (b) => this._OnPlay(b));
      this.AddDOMElementMessageHandler("pause", (b) => this._OnPause(b));
      this.AddDOMElementMessageHandler("set-source", (b, c) =>
        this._OnSetSource(b, c)
      );
      this.AddDOMElementMessageHandler("set-playback-time", (b, c) =>
        this._OnSetPlaybackTime(b, c)
      );
      this.AddDOMElementMessageHandler("set-playback-rate", (b, c) =>
        this._OnSetPlaybackRate(b, c)
      );
      this.AddDOMElementMessageHandler("set-looping", (b, c) =>
        this._OnSetLooping(b, c)
      );
      this.AddDOMElementMessageHandler("set-muted", (b, c) =>
        this._OnSetMuted(b, c)
      );
      this.AddDOMElementMessageHandler("set-volume", (b, c) =>
        this._OnSetVolume(b, c)
      );
      self.C3Video_GetElement = (b) => this.GetElementById(b);
    }
    async _OnInit(a) {
      this._isRuntimeInWorker = a.isInWorker;
      a = document.createElement("video");
      this._supportsRvfc =
        !!a.requestVideoFrameCallback && !this._iRuntime.IsAndroidWebView();
      const b = !!self.createImageBitmap;
      this._postImageBitmaps = this._isRuntimeInWorker || b;
      return {
        postImageBitmaps: this._postImageBitmaps,
        supportedFormats: {
          "webm-vp8": !!a.canPlayType("video/webm;codecs=vp8"),
          "webm-vp9": !!a.canPlayType("video/webm;codecs=vp9"),
          "webm-av1": !!a.canPlayType("video/webm;codecs=av01.0.00M.08"),
          "mp4-h264": !!a.canPlayType("video/mp4;codecs=avc1.420034"),
          "mp4-h265": !!a.canPlayType("video/mp4;codecs=hev1.1.2.L93.B0"),
          "mp4-av1": !!a.canPlayType("video/mp4;codecs=av01.0.00M.08"),
        },
      };
    }
    TryPlayMedia(a) {
      ...
      this._postImageBitmaps &&
        this._supportsRvfc &&
        -1 === a.c3_rvfcId &&
        (a.c3_rvfcId = a.requestVideoFrameCallback(() =>
          this._OnVideoFrameCallback(a)
        ));
    }
    RemovePendingPlay(a) {
      this._iRuntime.RemovePendingPlay(a);
      this._postImageBitmaps &&
        this._supportsRvfc &&
        -1 !== a.c3_rvfcId &&
        (a.cancelVideoFrameCallback(a.c3_rvfcId), (a.c3_rvfcId = -1));
    }
    async _OnVideoFrameCallback(a) {
      if (!this._destroyedVideos.has(a)) {
        a.c3_rvfcId = a.requestVideoFrameCallback(() =>
          this._OnVideoFrameCallback(a)
        );
        try {
          const b = a.c3_sequenceNumber++,
            c = await createImageBitmap(a);
          b > a.c3_lastFrameSequence &&
            ((a.c3_imageBitmap = c), (a.c3_lastFrameSequence = b));
        } catch (b) {}
      }
    }
    CreateElement(a, b) {
      ...
      c.playsInline = !0;
      c.c3_imageBitmap = null;
      c.c3_rvfcId = -1;
      c.c3_sequenceNumber = 0;
      c.c3_lastFrameSequence = -1;
      c.addEventListener("canplay", () => this._SendPlaybackEvent(c, a, 0));
      c.addEventListener("canplaythrough", () =>
        this._SendPlaybackEvent(c, a, 1)
      );
      c.addEventListener("ended", () => this._SendPlaybackEvent(c, a, 2));
      c.addEventListener("error", () => this._SendPlaybackEvent(c, a, 3));
      c.addEventListener("loadstart", () => this._SendPlaybackEvent(c, a, 4));
      c.addEventListener("playing", () => this._SendPlaybackEvent(c, a, 5));
      c.addEventListener("pause", () => this._SendPlaybackEvent(c, a, 6));
      c.addEventListener("stalled", () => this._SendPlaybackEvent(c, a, 7));
      const d = b.autoplay;
      0 === d
        ? ((c.autoplay = !1), (c.preload = "none"))
        : 1 === d
        ? ((c.autoplay = !1), (c.preload = "auto"))
        : (c.autoplay = !0);
      b.src && (c.src = b.src);
      2 === d && this.TryPlayMedia(c);
      this._numVideos++;
      this._StartTicking();
      return c;
    }
    _SendPlaybackEvent(a, b, c) {
      this._destroyedVideos.has(a) ||
        this.PostToRuntimeElement("playback-event", b, { type: c });
    }
    UpdateState(a, b) {}
    DestroyElement(a) {
      this.RemovePendingPlay(a);
      IsVideoPlaying(a) && a.pause();
      a.src = "";
      this._numVideos = Math.max(this._numVideos - 1, 0);
      0 === this._numVideos && this._StopTicking();
      this._destroyedVideos.add(a);
    }
    _OnPlay(a) {
      this.TryPlayMedia(a);
    }
    ...
    _OnSetPlaybackTime(a, b) {
      try {
        a.currentTime = b.time;
      } catch (c) {
        console.error("[Video] Exception setting playback time: ", c);
      }
    }
    _OnSetPlaybackRate(a, b) {
      try {
        a.playbackRate = b.rate;
      } catch (c) {
        console.error("[Video] Exception setting playback rate: ", c);
      }
    }
    _OnSetLooping(a, b) {
      a.loop = b.isLooping;
    ...
    Tick() {
      const a = {},
        b = { sequenceNumber: this._sequenceNumber++, videoData: a },
        c = [],
        d = [];
      for (const [e, f] of this._elementMap.entries()) {
        const g = f.GetElement(),
          h = {
        ...
        if (this._postImageBitmaps)
          if (this._supportsRvfc) {
            const k = g.c3_imageBitmap;
            g.c3_imageBitmap = null;
            (h.imageBitmap = k) && c.push(k);
          } else
            d.push(
              createImageBitmap(g)
                .then((k) => {
      ...
      Promise.all(d).then(() => {
        this._isRuntimeInWorker
          ? this.PostToRuntime("state", b, !1, c)
          : this._PostToRuntimeMaybeSync("state", b);
      });
    }
  }
);
