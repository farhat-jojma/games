...
("use strict");
{
  ...
  window.RuntimeInterface = class g {
    ...
    async _Init(c) {
      ...
      c.runtimeBaseUrl
        ? (this._runtimeBaseUrl = c.runtimeBaseUrl)
        : ((k = location.origin),
          (this._runtimeBaseUrl =
    ...
    SetIsExportingToVideo(c) {
      this._isExportingToVideo = !0;
      this._exportToVideoDuration = c;
    }
    IsExportingToVideo() {
    ...
    async _MaybeGetCordovaScriptURL(c) {
      return "cordova" === this._exportType &&
        ...
        ? (c.startsWith(this._runtimeBaseUrl) &&
            (c = c.substr(this._runtimeBaseUrl.length)),
...
self.JobSchedulerDOM = class {
  constructor(d) {
    ...
    this._baseUrl = d.GetRuntimeBaseURL();
